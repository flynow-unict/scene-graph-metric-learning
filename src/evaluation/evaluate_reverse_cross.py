...
def evaluate_reverse_cross():
    script_dir = Path(__file__).parent.resolve()
    base_dir = script_dir.parent.parent
    cross_dir = base_dir / "data" / "cross_evaluation_reverse"
    ...
    for m in gcn_models:
        configs[f"15k Baseline {m.upper()} -> FullSet TestSet"] = (
            cross_dir / f"baseline_test_queries_{m}.pt",
            cross_dir / f"baseline_test_gallery_{m}.pt",
            base_dir / "data" / "models" / "fullset" / "baseline" / "relevance_test.pt"
        )
        configs[f"15k SemanticWeb {m.upper()} -> FullSet TestSet"] = (
            cross_dir / f"semantic_test_queries_{m}.pt",
            cross_dir / f"semantic_test_gallery_{m}.pt",
            base_dir / "data" / "models" / "fullset" / "semantic_web" / "relevance_test.pt"
        )
    ...
    print("="*60)
    print(" VALUTAZIONE INVERSA (15K -> FULLSET) ".center(60))
    print("="*60)
    ...
    for model_name, (query_path, gallery_path, gt_path) in configs.items():
        ...
        # Carica Ground Truth del Full Set (1000x20215)
    ...
    with open(results_dir / "reverse_cross_evaluation_results.json", "w") as f:
        json.dump(results, f, indent=4)
        
    with open(results_dir / "reverse_cross_evaluation_results.md", "w") as f:
        f.write("# Risultati Cross-Evaluation Inversa: 15k Models -> Full Set Test Set\n\n")
        f.write("Questo report valuta la robustezza dei modelli addestrati sul sotto-dataset (15k) applicati alle immagini giganti del Full Set (55k).\n\n")
    ...
    print(f"\n[✔] Valutazione completata. Risultati salvati in results/reverse_cross_evaluation_results.md")

if __name__ == "__main__":
    evaluate_reverse_cross()
