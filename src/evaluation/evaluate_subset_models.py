...
def evaluate_subset():
    ...
    # Ground truths
    rel_baseline = subset_dir / "baseline" / "relevance_test.pt"
    rel_semantic = subset_dir / "semantic_web" / "relevance_test.pt"
    
    k_values = [1, 5, 10, 20]
    results = {}
    
    # Mappa dei modelli da valutare (nome_visuale: (percorso_query, percorso_indice_faiss, relevance))
    configs = {
        # 1. Semantic Web GCN (Rel)
        "Semantic Web GCN (GINE)": (
            subset_dir / "semantic_web" / "gcn" / "gcn_gine_test_queries.pt",
            faiss_dir / "faiss_gcn_gine.index",
            rel_semantic
        ),
        "Semantic Web GCN (SAGE)": (
            subset_dir / "semantic_web" / "gcn" / "gcn_sage_test_queries.pt",
            faiss_dir / "faiss_gcn_sage.index",
            rel_semantic
        ),
        "Semantic Web GCN (GINE TRIPLET)": (
            subset_dir / "semantic_web" / "gcn" / "gcn_gine_triplet_test_queries.pt",
            faiss_dir / "faiss_gcn_gine_triplet.index",
            rel_semantic
        ),
        "Semantic Web GCN (SAGE TRIPLET)": (
            subset_dir / "semantic_web" / "gcn" / "gcn_sage_triplet_test_queries.pt",
            faiss_dir / "faiss_gcn_sage_triplet.index",
            rel_semantic
        ),
        # 2. Baseline GCN (NoRel)
        "Baseline GCN (GINE)": (
            subset_dir / "baseline" / "gcn" / "baseline_gcn_test_queries_gine.pt",
            faiss_dir / "baseline_gcn_gine.index",
            rel_baseline
        ),
        "Baseline GCN (SAGE)": (
            subset_dir / "baseline" / "gcn" / "baseline_gcn_test_queries_sage.pt",
            faiss_dir / "baseline_gcn_sage.index",
            rel_baseline
        ),
        "Baseline GCN (GINE TRIPLET)": (
            subset_dir / "baseline" / "gcn" / "baseline_gcn_test_queries_gine_triplet.pt",
            faiss_dir / "baseline_gcn_gine_triplet.index",
            rel_baseline
        ),
        "Baseline GCN (SAGE TRIPLET)": (
            subset_dir / "baseline" / "gcn" / "baseline_gcn_test_queries_sage_triplet.pt",
            faiss_dir / "baseline_gcn_sage_triplet.index",
            rel_baseline
        ),
        # 3. Vision Baselines
        "Vision Baseline (CLIP)": (
            subset_dir / "baseline" / "vision" / "clip_test_queries.pt",
            faiss_dir / "vision_clip.index",
            rel_baseline
        ),
        "Vision Baseline (RESNET)": (
            subset_dir / "baseline" / "vision" / "resnet_test_queries.pt",
    ...
    print("="*60)
    print(" VALUTAZIONE RETRIEVAL - SUBSET (15k) ".center(60))
    print("="*60)
    ...
    for model_name, (query_path, index_path, relevance_path) in configs.items():
        ...
        if not relevance_path.exists():
            print(f"[!] ERRORE: Ground truth mancante in {relevance_path}")
            continue
    ...
    output_json = results_dir / "subset_evaluation_results.json"
    with open(output_json, "w") as f:
        json.dump(results, f, indent=4)
        
    # Creazione di un file Markdown leggibile
    output_md = results_dir / "subset_evaluation_results.md"
    with open(output_md, "w") as f:
        f.write("# Risultati Retrieval - Sotto-Dataset (Subset 15k)\n\n")
        f.write("Questo report confronta le GCN (Baseline vs Semantic Web) e le Vision Baselines sul sotto-dataset a 15k.\n\n")
